...
* [Mybatis-Plus 我来填坑~](#mybatis-plus-我来填坑)
      * [目录:](#目录)
          * [简介：](#简介：)
  ...
  * [一、简单介绍](#一、简单介绍)
    ...
    * [成绩：](#成绩：)
    * [最新版本：](#最新版本：)
    * [开发层面MyBatis-Plus特色](#开发层面mybatis-plus特色)
    * [Mybatis-Plus中的Plus](#mybatis-plus中的plus)
  * [二、MP的特性](#二、mp的特性)
  * [三、MP框架结构](#三、mp框架结构)
  * [四、简单的入门Demo(Mysql)](#四、简单的入门demomysql)
      * [Demo代码地址：](#demo代码地址：)
      * [Demo 环境：](#demo-环境：)
      * [初始化：](#初始化：)
        * [数据库：MySql](#数据库：mysql)
      ...
      * [工程：](#工程：)
        * [第一步、创建工程](#第一步、创建工程)
        * [第二步、引入依赖坐标](#第二步、引入依赖坐标)
        * [第三步、配置数据源](#第三步、配置数据源)
* [DataSource Config](#datasource-config)
* [这里如果有错误是因为 maven mysql包 选择了 runtime 形式的 scope  可以不用管它 继续下一步就好](#这里如果有错误是因为-maven-mysql包-选择了-runtime-形式的-scope--可以不用管它-继续下一步就好)
    ​    * [第四步、添加mybatis扫描位置](#第四步、添加mybatis扫描位置)
    ​    * [第五步、pojo及mapper](#第五步、pojo及mapper)
    ​    * [第六步、测试](#第六步、测试)
  * [五、核心功能](#五、核心功能)
    * [核心一-简便之-代码生成器（AutoGenerator）](#核心一-简便之-代码生成器（autogenerator）)
      * [Demo代码地址：](#demo代码地址：-1)
      * [Demo 环境：](#demo-环境：-1)
      * [工程：](#工程：-1)
        * [第一步、创建模块](#第一步、创建模块)
        * [第二步、引入依赖坐标](#第二步、引入依赖坐标-1)
        * [第三步、编写代码生成器](#第三步、编写代码生成器)
        * [第四步、运行测试](#第四步、运行测试)
        * [**发现问题**： 如果是多模块项目 生成的文件会直接到了父项目目录下](#发现问题：-如果是多模块项目-生成的文件会直接到了父项目目录下)
          * [解决方法：](#解决方法：)
    * [核心二 - 清晰之-CRUD接口](#核心二---清晰之-crud接口)
      * [Demo代码地址：](#demo代码地址：-2)
      * [工程：](#工程：-2)
        * [第一步、创建模块](#第一步、创建模块-1)
        * [第二步、引入依赖坐标](#第二步、引入依赖坐标-2)
        * [第三步、生成代码](#第三步、生成代码)
        * [第四步、其他配置需要](#第四步、其他配置需要)
* [DataSource Config](#datasource-config-1)
* [这里如果有错误是因为 maven mysql包 选择了 runtime 形式的 scope  可以不用管它 继续下一步就好](#这里如果有错误是因为-maven-mysql包-选择了-runtime-形式的-scope--可以不用管它-继续下一步就好-1)
    ​    * [第五步、对MP探个究竟！](#第五步、对mp探个究竟！)
    ​    * [**发现问题**:  我虽然看到了方法 但是并没有一条SQL 那么它是怎样做到查询的呢?](#发现问题--我虽然看到了方法-但是并没有一条sql-那么它是怎样做到查询的呢)
    ​      * [探索:](#探索)
    ​    * [第六步、使用它](#第六步、使用它)
    ​      * [Mapper - 简单的CRUD:](#mapper---简单的crud)
    ​    * [**发现问题**: 虽然MP替我生成了 uuid 作为主键,但是还是想用数据库自增形式主键怎么办?](#发现问题-虽然mp替我生成了-uuid-作为主键但是还是想用数据库自增形式主键怎么办)
    ​      * [解决:](#解决)
    ​      * [Mapper - 批量的CRUD接口:](#mapper---批量的crud接口)
    ​      * [Mapper - 选装组件:](#mapper---选装组件)
    ​      * [Service - CRUD:](#service---crud)
    * [核心三 - 强大之 -Wrapper条件构造器](#核心三---强大之--wrapper条件构造器)
      * [Demo代码地址：](#demo代码地址：-3)
      * [工程:](#工程)
        * [第一 -->第四步 同上一工程步骤](#第一---第四步-同上一工程步骤)
        * [第五步、探索Wrapper](#第五步、探索wrapper)
          * [Wrapper含义:](#wrapper含义)
          * [版本不同:](#版本不同)
          * [3.x Wrapper主要继承结构:](#3x-wrapper主要继承结构)
          * [AbstractWrapper接口:](#abstractwrapper接口)
          * [AbstractWrapper接口:](#abstractwrapper接口-1)
          * [UpdateWrapper接口:](#updatewrapper接口)
        * [第六步、使用接口](#第六步、使用接口)
          * [一> 编写AbstractWrapper测试类:](#一-编写abstractwrapper测试类)
        * [**发现问题**: `boolean condition` 、` R column`和 `Function<This, This> func` 三个参数相关问题](#发现问题-`boolean-condition`-、`-r-column`和-`functionthis-this-func`-三个参数相关问题)
          * [说明及使用:](#说明及使用)
          * [1.关于入参: `boolean condition`](#1关于入参-`boolean-condition`)
          * [2.关于入参: `R column`](#2关于入参-`r-column`)
          * [3.关于入参: `Function<This, This> func`](#3关于入参-`functionthis-this-func`)
          * [二> 编写QueryWrapper与UpdateWrapper测试类:](#二-编写querywrapper与updatewrapper测试类)
        * [**注意坑**  在借用 `lambda()` 使用 lambda特性时!](#注意坑--在借用-`lambda`-使用-lambda特性时)
  * [六、插件及扩展](#六、插件及扩展)
    * [分页插件:](#分页插件)
    * [热加载:](#热加载)
    * [逻辑删除:](#逻辑删除)
